  ...
  <header style="background: var(--back-color); color: var(--back-text-color)">
    ...
    <div use:slideIn class="{grid({ columns: 1, gap: 'horizontal' })}">
      ...
      <aside>
        ...
        {#if contributor.fields.job}{contributor.fields.job}&nbsp;{/if}{#if contributor.fields.contactLink}<a href={contributor.fields.contactLink} target="_blank" rel="external">{contributor.fields.title}</a>{:else}{contributor.fields.title}{/if}{#if contributor.fields.description}, {contributor.fields.description}{/if}{#if i < article.fields.contributors.length - 1},&nbsp;{/if}
        {/each}
        {/if}
      </aside>
