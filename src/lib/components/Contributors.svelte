...
<div use:slideIn class="contributors {grid({ columns: 2, gap: 'horizontal' })} {col({ span: 2 })}">
  <h3 class="{col({ span: 2 })}">{$page.params.locale === 'fr' ? contributors.length === 1 ? 'Contributeur·trice' : 'Contributeurs·trices' : contributors.length === 1 ? 'Contributor' : 'Contributors'}</h3>
